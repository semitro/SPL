...
enum read_status from_bmp(FILE *in, struct image* img){
	...
	header_prototype.biWidth = abs(header.biWidth);
	header_prototype.biHeight = abs(header.biHeight);
...
struct bmp_header generate_header(const struct image * img){
	...
	header_prototype.biSizeImage = img->height * img->width * sizeof(struct pixel)
                        + ((BMP_PADDING-(img->width*sizeof(struct pixel))%BMP_PADDING) %BMP_PADDING)*img->height;
